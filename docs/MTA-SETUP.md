...
- **Keycloak-backed SASL (A9)** — Dovecot SASL-only (`protocols =`, no IMAP) in Postfix container with `passdb { driver = checkpassword; args = /usr/local/bin/kc-verify.sh }`. Script reads NUL-delimited creds from fd 3 (via `IFS= read -rd ''` to preserve null bytes) → curl Keycloak `/realms/${KC_REALM}/protocol/openid-connect/token` with `grant_type=password` + client secret → exit 0 on HTTP 200, 111 on 5xx, 1 otherwise. Dovecot socket at `/var/spool/postfix/private/auth` (mode 0660 postfix:postfix). Postfix switched from Cyrus sasldb2 → `smtpd_sasl_type=dovecot smtpd_sasl_path=private/auth`. Dovecot `import_environment` extended with `KC_URL KC_REALM KC_CLIENT_ID KC_CLIENT_SECRET`. Entrypoint starts Dovecot in background, cleans stale socket on restart.
  - Validated: `alice`/`alice123` → 235 Authentication successful → milter logs `outbound DKIM signed user="alice"` → LMTP receipt bytes=989. Wrong password → 535 rejection.
