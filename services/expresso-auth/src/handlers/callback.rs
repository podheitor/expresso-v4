...
pub async fn callback(
    ...
    let ctx =
        if let (Some(realm), Some(mv)) = (pending.realm.as_deref(), app.multi_validator.as_ref()) {
    ...
    if json_mode {
        ...
        resp.headers_mut()
            .append(SET_COOKIE, at_cookie.parse().unwrap());
...
mod tests {
    ...
    fn accepts_path_with_query_param_and_hash() {
        assert!(is_safe_local_redirect(
            "/settings?tab=security#notifications"
        ));
    }
