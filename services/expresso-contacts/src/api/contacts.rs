...
async fn create_inner(
    ...
    let c = ContactRepo::new(pool)
        .create(ctx.tenant_id, book_id, &raw)
        .await?;
...
async fn list_addresses(
    ...
    ctx: RequestCtx,
    Path((_book_id, id)): Path<(Uuid, Uuid)>,
) -> Result<axum::Json<Vec<crate::domain::ContactAddressRow>>> {
    let pool = state.db_or_unavailable()?;
    let repo = ContactRepo::new(pool);
    repo.get(ctx.tenant_id, id).await?; // 404 guard
    let addresses = repo.list_addresses(ctx.tenant_id, id).await?;
    Ok(axum::Json(addresses))
}
...
async fn export_vcf_inner(
    ...
    Ok(Response::builder()
        ...
        .header(
            ...
            "attachment; filename=\"addressbook.vcf\"",
        )
        .body(Body::from(body))
        .unwrap())
}
...
async fn import_vcf(
    ...
    if blocks.is_empty() {
        return Err(crate::error::ContactsError::InvalidVCard(
            "no VCARD blocks found".into(),
        ));
    }
    let pool = state.db_or_unavailable()?;
    let repo = ContactRepo::new(pool);
...
async fn import_csv(
    ...
    let book_id: Uuid = book_id_raw
        ...
        .and_then(|s| {
            s.parse::<Uuid>()
                .map_err(|_| ContactsError::BadRequest("invalid book_id".into()))
        })?;

    let csv_data =
        csv_bytes.ok_or_else(|| ContactsError::BadRequest("missing file field".into()))?;
...
mod tests {
    ...
    fn rejects_empty() {
        let err = format!(
            "{:?}",
            validate_vcard("", MAX_CONTACT_VCARD_BYTES).unwrap_err()
        );
    ...
    fn rejects_oversize_contact() {
        ...
        let err = format!(
            "{:?}",
            validate_vcard(&s, MAX_CONTACT_VCARD_BYTES).unwrap_err()
        );
    ...
    #[test]
    fn max_contact_vcard_bytes_is_power_of_two() {
        assert_eq!(MAX_CONTACT_VCARD_BYTES & (MAX_CONTACT_VCARD_BYTES - 1), 0);
    }
