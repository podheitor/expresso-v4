...
## Anti-reverse-engineering hardening (release builds)

This project ships proprietary code (`LicenseRef-PodHeitor-Proprietary`). Release builds should make reverse engineering *substantially* harder without claiming impossible guarantees — a determined attacker with a debugger still wins; the goal is to raise the cost.

### 1. Aggressive release profile

`Cargo.toml` `[profile.release]` should set:

```toml
[profile.release]
strip = true          # drop symbol tables from the binary
lto = true            # link-time optimisation — also removes dead code that leaks intent
codegen-units = 1     # one TU → harder to reason about function boundaries
panic = "abort"       # no unwind tables (which also leak source-level type/location info)
```

`debug = false` is already the release default; don't override it. Keep `opt-level = 3` (default).

Trade-off: longer link time (LTO + codegen-units=1) and no panic backtraces in production. For a daemon this is the right call — operators read structured logs, not stack traces.

### 2. Symbol strip (belt-and-braces)

Even with `strip = true` in the manifest, run `strip target/release/<binary>` as a packaging step (PKGBUILD, deb postinst, etc.). Catches binaries built with older toolchains and any debuginfo split files.

### 3. Sensitive strings hygiene

- Don't ship verbose internal error messages, source file paths, or wire-protocol literals in release. Stock `tracing` calls at `debug!`/`trace!` should be gated behind a `debug-logs` feature flag that is **off** in release packages.
- Audit `strings <binary>` before a release cut. Anything that names internal modules, secrets layout, or proprietary algorithms should be hidden behind feature flags or constant-folded out.
- Format-string literals in `panic!`/`unreachable!`/`assert!` are sticky — they survive strip. Prefer `unreachable!()` over `unreachable!("got X with field Y")` in release-critical paths.

### 4. Encapsulate the crown jewels

The modules a competitor would steal from first — crypto routines, scheduler heuristics, deduplication, compression framing — should live in self-contained crates with the smallest possible `pub` surface. Hide implementation details behind opaque types (`pub struct X { inner: Inner }`) so even with symbols you'd need to reconstruct the type layout to use the function.

### 5. Licensing posture (not just a code question)

The free build is **not** open source. Every release artefact should ship:

- A `LICENSE` file with the proprietary text (or a pointer to it).
- An EULA the user clicks through (installer / first-run prompt) covering: anti-reverse-engineering clause, redistribution rules, support scope.
- Clear separation in marketing/docs between "free as in price" and "open source" — never imply the latter.

Anti-reverse-engineering clauses have varying enforceability across jurisdictions (notably the EU's Software Directive 2009/24/EC permits interop reverse engineering despite contracts) — the clause is still worth having as a deterrent and as an evidentiary baseline for civil action.

### Don't pursue:

- Custom packers / runtime decryptors. They add a few hours of attacker time and a lot of operational pain (debugging in prod becomes hellish, AV vendors flag them).
- Obfuscated control flow at the Rust level. The compiler will fight you and the maintainability cost is permanent.
- Anti-debug syscalls / `ptrace` traps. Easy to bypass; trips honest debugging.

Hardening is a real cost-benefit calculation. The four steps above raise the bar enough that casual reverse engineers move on; anything beyond that is theatre.
