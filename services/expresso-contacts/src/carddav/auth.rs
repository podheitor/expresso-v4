//! CardDAV HTTP Basic authentication.
//!
...
//! 2. Else if `CONTACTS_DEV_AUTH=1` → dev mode: resolve user by email, accept
...
async fn resolve_user(
...
) -> Result<CardDavPrincipal, AuthError> {
    ...
    let rows: Vec<(Uuid, Uuid)> =
        sqlx::query_as(r#"SELECT tenant_id, id FROM users WHERE email = $1 LIMIT 2"#)
            ...
            .map_err(|_| AuthError::Unavailable)?;
    match rows.as_slice() {
        [(tenant_id, user_id)] => Ok(CardDavPrincipal {
            tenant_id: *tenant_id,
            user_id: *user_id,
        }),
        [] => Err(AuthError::Forbidden),
        _ => {
            tracing::warn!(email = %user, "ambiguous CardDAV login: email exists in multiple tenants and no Host→tenant mapping wired");
            Err(AuthError::Forbidden)
        }
    }
}
...
impl IntoResponse for AuthError {
    fn into_response(self) -> Response {
        let challenge =
            HeaderValue::from_static("Basic realm=\"Expresso CardDAV\", charset=\"UTF-8\"");
