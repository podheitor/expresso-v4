...
pub enum RpError {
    ...
    #[error(transparent)]
    Auth(#[from] expresso_auth_client::AuthError),
    #[error(transparent)]
...
mod tests {
    ...
    fn token_exchange_failure_is_502() {
        assert_eq!(
            status(RpError::TokenExchange("err".into())),
    ...
    fn refresh_failure_is_401() {
        assert_eq!(
            status(RpError::Refresh("err".into())),
    ...
    fn discovery_failure_is_503() {
        assert_eq!(
            status(RpError::Discovery("err".into())),
    ...
    fn config_error_is_500() {
        assert_eq!(
            status(RpError::Config("err".into())),
    ...
    fn bad_request_static_str_is_400() {
        assert_eq!(
            status(RpError::BadRequest("invalid_param")),
    ...
    fn bad_request_invalid_state_is_400() {
        assert_eq!(
            status(RpError::BadRequest("invalid_state")),
            StatusCode::BAD_REQUEST
        );
    }
    ...
    fn config_missing_secret_is_500() {
        assert_eq!(
            status(RpError::Config("missing_secret".into())),
            StatusCode::INTERNAL_SERVER_ERROR
        );
    }
    ...
    fn token_exchange_error_is_bad_gateway() {
        assert_eq!(
            status(RpError::TokenExchange("upstream failed".into())),
            StatusCode::BAD_GATEWAY
        );
    }
    ...
    fn discovery_error_is_service_unavailable() {
        assert_eq!(
            status(RpError::Discovery("no endpoint".into())),
            StatusCode::SERVICE_UNAVAILABLE
        );
    }
    ...
    fn auth_error_is_unauthorized() {
        assert_eq!(
            status(RpError::Auth(
                expresso_auth_client::AuthError::InvalidToken("invalid token".into())
            )),
            StatusCode::UNAUTHORIZED,
        );
    }
    ...
    fn refresh_error_is_unauthorized() {
        assert_eq!(
            status(RpError::Refresh("expired".into())),
            StatusCode::UNAUTHORIZED
        );
    }
    ...
    fn config_error_status_is_500() {
        assert_eq!(
            status(RpError::Config("missing var".into())),
            StatusCode::INTERNAL_SERVER_ERROR
        );
    }
    ...
    fn refresh_error_status_is_401() {
        assert_eq!(
            status(RpError::Refresh("timeout".into())),
            StatusCode::UNAUTHORIZED
        );
    }
