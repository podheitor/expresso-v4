...
pub async fn forgot(state: State<Arc<AppState>>, req: Json<ForgotReq>) -> StatusCode {
    let started = Instant::now();
    // Always 204 (no user-existence leak); `ok` tracks backend health, not the
    // HTTP status — a KC lookup/email failure is an error even though the client
    // still sees 204.
    let ok = forgot_inner(state, req).await;
    crate::metrics::record(crate::metrics::OP_FORGOT, started, ok);
    StatusCode::NO_CONTENT
}

async fn forgot_inner(State(state): State<Arc<AppState>>, Json(req): Json<ForgotReq>) -> bool {
