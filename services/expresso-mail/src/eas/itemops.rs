...
/// One parsed Fetch request entry.
...
/// carry a CollectionId + ServerId; we collect every complete pair.
pub fn parse_fetches(body: &[u8]) -> Vec<FetchReq> {
    ...
    for ev in events {
        match &ev {
            Event::StartElement { page: p, token, .. } if *p == page::ITEM_OPERATIONS => {
                match *token {
                    iop::FETCH => {
                        in_fetch = true;
                        collection_id = None;
                    ...
                    iop::COLLECTION_ID => want = Some(iop::COLLECTION_ID),
                    iop::SERVER_ID => want = Some(iop::SERVER_ID),
                    _ => want = None,
                }
            }
            Event::Text(t) if in_fetch => {
                match want {
                    Some(iop::COLLECTION_ID) => collection_id = Some(t.clone()),
                    Some(iop::SERVER_ID) => server_id = Some(t.clone()),
                    _ => {}
                }
                want = None;
            }
            Event::EndElement => {
                if in_fetch {
                    if let (Some(c), Some(s)) = (collection_id.clone(), server_id.clone()) {
                        out.push(FetchReq {
                            collection_id: c,
                            server_id: s,
                        });
...
pub async fn item_operations_response(
    ...
    for f in fetches {
        ...
        doc.push(Event::start(i, iop::STATUS));
        doc.push(Event::Text(if body.is_some() { "1" } else { "15" }.into())); // 15 = not found
        doc.push(Event::EndElement);
        doc.push(Event::start(i, iop::COLLECTION_ID));
        doc.push(Event::Text(f.collection_id.clone()));
        doc.push(Event::EndElement);
        doc.push(Event::start(i, iop::SERVER_ID));
        doc.push(Event::Text(f.server_id.clone()));
        doc.push(Event::EndElement);
        if let Some(text) = body {
            doc.push(Event::start(i, iop::PROPERTIES));
            doc.push(Event::start(b, air_sync_base::BODY));
            push_text(&mut doc, b, air_sync_base::TYPE, "1");
            push_text(&mut doc, b, air_sync_base::TRUNCATED, "0");
            push_text(&mut doc, b, air_sync_base::DATA, &text);
...
async fn fetch_body_text(state: &AppState, tenant_id: Uuid, f: &FetchReq) -> Option<String> {
    let uid: i64 = f.server_id.rsplit_once(':')?.1.parse().ok()?;
    let mailbox_id = Uuid::parse_str(&f.collection_id).ok()?;
...
mod tests {
    ...
    fn parse_fetches_reads_pair() {
        ...
        assert_eq!(
            parse_fetches(&body),
            vec![FetchReq {
                collection_id: "mbox-1".into(),
                server_id: "mbox-1:42".into()
            }]
        );
    }
    ...
    fn parse_fetches_two_entries() {
        ...
        assert_eq!(got.len(), 2);
        assert_eq!(got[1].server_id, "m2:2");
    }
}
