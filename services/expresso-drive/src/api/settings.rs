...
mod tests {
    ...
    #[test]
    fn trash_purge_put_body_zero_days() {
        let b: TrashPurgePutBody = serde_json::from_str(r#"{"auto_purge_days":0}"#).unwrap();
        assert_eq!(b.auto_purge_days, Some(0));
    }
    ...
    #[test]
    fn trash_purge_put_body_none_when_absent() {
        let b: TrashPurgePutBody = serde_json::from_str(r#"{}"#).unwrap();
        assert!(b.auto_purge_days.is_none());
    }
}
