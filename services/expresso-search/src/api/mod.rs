...
/// GET /api/v1/search/index/segments/size-below-mean — alias semântico para bytes abaixo da média. Sprint #5045.
pub async fn segment_size_below_mean(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"mean_bytes": null, "rows": [], "count_below": 0, "segment_count": 0}));
    }
    let total_bytes: u64 = segs.iter().map(|(_, _, db)| *db).sum();
    let mean = total_bytes as f64 / n as f64;
    let below: Vec<serde_json::Value> = segs.iter()
        ...
        .map(|(id, nd, db)| serde_json::json!({"id": id, "num_docs": nd, "disk_bytes": db}))
        .collect();
    let count_below = below.len();
    Json(serde_json::json!({"mean_bytes": mean, "rows": below, "count_below": count_below, "segment_count": n}))
}

/// GET /api/v1/search/index/segments/docs-below-mean — segmentos com num_docs < média. Sprint #5046.
pub async fn segment_docs_below_mean(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"mean_docs": null, "rows": [], "count_below": 0, "segment_count": 0}));
    }
    let total_docs: u64 = segs.iter().map(|(_, nd, _)| *nd).sum();
    let mean = total_docs as f64 / n as f64;
    let below: Vec<serde_json::Value> = segs.iter()
        ...
        .collect();
    let count_below = below.len();
    Json(serde_json::json!({"mean_docs": mean, "rows": below, "count_below": count_below, "segment_count": n}))
}

/// GET /api/v1/search/index/segments/bytes-below-mean — segmentos com disk_bytes < média. Sprint #5047.
pub async fn segment_bytes_below_mean(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"mean_bytes": null, "rows": [], "count_below": 0, "segment_count": 0}));
...
/// GET /api/v1/search/index/segments/size-above-p01 — segmentos com disk_bytes > P01. Sprint #5048.
pub async fn segment_size_above_p01(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
    ...
    let above_count = segs.iter().filter(|(_, _, db)| *db > p01).count();
    let ratio = above_count as f64 / n as f64;
    Json(serde_json::json!({"p01_bytes": p01, "above_count": above_count, "above_ratio": ratio, "segment_count": n}))
}

/// GET /api/v1/search/index/segments/size-below-p01 — segmentos com disk_bytes <= P01. Sprint #5049.
pub async fn segment_size_below_p01(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
...
/// GET /api/v1/search/index/segments/size-above-p05 — segmentos com disk_bytes > P05. Sprint #5050.
pub async fn segment_size_above_p05(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
    ...
    let above_count = segs.iter().filter(|(_, _, db)| *db > p05).count();
    let ratio = above_count as f64 / n as f64;
    Json(serde_json::json!({"p05_bytes": p05, "above_count": above_count, "above_ratio": ratio, "segment_count": n}))
}

/// GET /api/v1/search/index/segments/size-below-p05 — segmentos com disk_bytes <= P05. Sprint #5051.
pub async fn segment_size_below_p05(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
...
/// GET /api/v1/search/index/segments/docs-sum-above-p50 — segmentos com docs_sum > P50. Sprint #5052.
pub async fn segment_docs_sum_above_p50(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"p50_docs": null, "above_count": 0, "total_segments": 0}));
    }
    let mut vals: Vec<u64> = segs.iter().map(|(_, nd, _)| *nd).collect();
    vals.sort_unstable();
    let p50 = vals[n / 2];
    let count = vals.iter().filter(|&&v| v > p50).count();
    Json(serde_json::json!({"count_above_p50": count, "p50_docs": p50, "total_segments": n}))
}

/// GET /api/v1/search/index/segments/docs-above-p01 — segmentos com num_docs > P01. Sprint #5065.
pub async fn segment_docs_above_p01(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"p01_docs": null, "above_count": 0, "segment_count": 0}));
    }
    let mut docs: Vec<u64> = segs.iter().map(|(_, nd, _)| *nd).collect();
    docs.sort_unstable();
    let p01 = docs[(n * 1) / 100];
    let above_count = segs.iter().filter(|(_, nd, _)| *nd > p01).count();
    let ratio = above_count as f64 / n as f64;
    Json(serde_json::json!({"p01_docs": p01, "above_count": above_count, "above_ratio": ratio, "segment_count": n}))
}

/// GET /api/v1/search/index/segments/docs-below-p01 — segmentos com num_docs <= P01. Sprint #5066.
pub async fn segment_docs_below_p01(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"p01_docs": null, "below_count": 0, "segment_count": 0}));
    }
    let mut docs: Vec<u64> = segs.iter().map(|(_, nd, _)| *nd).collect();
    docs.sort_unstable();
    let p01 = docs[(n * 1) / 100];
    let below: Vec<serde_json::Value> = segs
        .iter()
        .filter(|(_, nd, _)| *nd <= p01)
        .map(|(id, nd, db)| serde_json::json!({"segment_id": id, "num_docs": nd, "disk_bytes": db}))
        .collect();
    Json(serde_json::json!({"p01_docs": p01, "below_count": below.len(), "segment_count": n, "segments": below}))
}

/// GET /api/v1/search/index/segments/docs-above-p05 — segmentos com num_docs > P05. Sprint #5067.
pub async fn segment_docs_above_p05(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"p05_docs": null, "above_count": 0, "segment_count": 0}));
    }
    let mut docs: Vec<u64> = segs.iter().map(|(_, nd, _)| *nd).collect();
    docs.sort_unstable();
    let p05 = docs[(n * 5) / 100];
    let above_count = segs.iter().filter(|(_, nd, _)| *nd > p05).count();
    let ratio = above_count as f64 / n as f64;
    Json(serde_json::json!({"p05_docs": p05, "above_count": above_count, "above_ratio": ratio, "segment_count": n}))
}

/// GET /api/v1/search/index/segments/docs-below-p05 — segmentos com num_docs <= P05. Sprint #5068.
pub async fn segment_docs_below_p05(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"p05_docs": null, "below_count": 0, "segment_count": 0}));
    }
    let mut docs: Vec<u64> = segs.iter().map(|(_, nd, _)| *nd).collect();
    docs.sort_unstable();
    let p05 = docs[(n * 5) / 100];
    let below: Vec<serde_json::Value> = segs
        .iter()
        .filter(|(_, nd, _)| *nd <= p05)
        .map(|(id, nd, db)| serde_json::json!({"segment_id": id, "num_docs": nd, "disk_bytes": db}))
        .collect();
    Json(serde_json::json!({"p05_docs": p05, "below_count": below.len(), "segment_count": n, "segments": below}))
}

/// GET /api/v1/search/index/segments/docs-above-p10 — segmentos com num_docs > P10. Sprint #5069.
pub async fn segment_docs_above_p10(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    ...
    let above_count = segs.iter().filter(|(_, nd, _)| *nd > p10).count();
    let ratio = above_count as f64 / n as f64;
    Json(serde_json::json!({"p10_docs": p10, "above_count": above_count, "above_ratio": ratio, "segment_count": n}))
}

/// GET /api/v1/search/index/segments/docs-below-p10 — segmentos com num_docs <= P10. Sprint #5070.
...
/// GET /api/v1/search/index/segments/docs-above-p25 — segmentos com num_docs > P25. Sprint #5071.
...
/// GET /api/v1/search/index/segments/docs-below-p25 — segmentos com num_docs <= P25. Sprint #5072.
...
pub async fn segment_docs_below_mean_zscore(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    ...
    let rows: Vec<serde_json::Value> = segs.iter().zip(vals.iter())
        ...
        .collect();
    Json(serde_json::json!({"mean_docs": mean, "stddev_docs": stddev, "below_mean_count": rows.len(), "segment_count": n, "segments": rows}))
}

/// GET /api/v1/search/index/segments/size-zscore-above-1 — segmentos com z-score de disk_bytes > 1 (alias size). Sprint #5146.
pub async fn segment_size_zscore_above_1(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"mean": null, "stddev": null, "above": [], "segment_count": 0}));
    }
    let vals: Vec<f64> = segs.iter().map(|(_, _, db)| *db as f64).collect();
    let mean = vals.iter().sum::<f64>() / n as f64;
    let stddev = (vals.iter().map(|v| (v - mean).powi(2)).sum::<f64>() / n as f64).sqrt();
    let above: Vec<serde_json::Value> = segs.iter()
        .filter(|(_, _, db)| stddev > 0.0 && (*db as f64 - mean) / stddev > 1.0)
        .map(|(id, nd, db)| serde_json::json!({"segment_id": id, "num_docs": nd, "disk_bytes": db}))
        .collect();
    Json(serde_json::json!({"mean_size": mean, "stddev_size": stddev, "above_count": above.len(), "segment_count": n, "segments": above}))
}

/// GET /api/v1/search/index/segments/size-zscore-below-1 — segmentos com z-score de disk_bytes < -1 (alias size). Sprint #5147.
pub async fn segment_size_zscore_below_1(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
    ...
    let below: Vec<serde_json::Value> = segs.iter()
        ...
        .map(|(id, nd, db)| serde_json::json!({"segment_id": id, "num_docs": nd, "disk_bytes": db}))
        .collect();
    Json(serde_json::json!({"mean_size": mean, "stddev_size": stddev, "below_count": below.len(), "segment_count": n, "segments": below}))
}

/// GET /api/v1/search/index/segments/size-zscore-above-2 — segmentos com z-score de disk_bytes > 2. Sprint #5148.
pub async fn segment_size_zscore_above_2(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"mean": null, "stddev": null, "above": [], "segment_count": 0}));
...
/// GET /api/v1/search/index/segments/size-zscore-below-2 — segmentos com z-score de disk_bytes < -2. Sprint #5149.
pub async fn segment_size_zscore_below_2(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"mean": null, "stddev": null, "below": [], "segment_count": 0}));
...
/// GET /api/v1/search/index/segments/size-mad-outliers — segmentos com |disk_bytes - median| > 3 * MAD (alias size). Sprint #5150.
pub async fn segment_size_mad_outliers(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
    ...
    let outliers: Vec<serde_json::Value> = segs.iter()
        ...
        .map(|(id, nd, db)| serde_json::json!({"segment_id": id, "num_docs": nd, "disk_bytes": db}))
        .collect();
    Json(serde_json::json!({"median_size": median, "mad": mad, "threshold": threshold, "outlier_count": outliers.len(), "segment_count": n, "outliers": outliers}))
}

/// GET /api/v1/search/index/segments/size-iqr-outliers — segmentos com disk_bytes fora de [Q1-1.5*IQR, Q3+1.5*IQR] (alias size). Sprint #5151.
pub async fn segment_size_iqr_outliers(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"q1": null, "q3": null, "iqr": null, "outliers": [], "segment_count": 0}));
    }
    let mut bytes: Vec<u64> = segs.iter().map(|(_, _, db)| *db).collect();
    bytes.sort_unstable();
    let q1 = bytes[(n * 25) / 100] as f64;
    let q3 = bytes[(n * 75) / 100] as f64;
    let iqr = q3 - q1;
    let lo = q1 - 1.5 * iqr;
    let hi = q3 + 1.5 * iqr;
    let outliers: Vec<serde_json::Value> = segs.iter()
        .filter(|(_, _, db)| (*db as f64) < lo || (*db as f64) > hi)
        .map(|(id, nd, db)| serde_json::json!({"segment_id": id, "num_docs": nd, "disk_bytes": db}))
        .collect();
    Json(serde_json::json!({"q1": q1, "q3": q3, "iqr": iqr, "fence_lo": lo, "fence_hi": hi, "outlier_count": outliers.len(), "segment_count": n, "outliers": outliers}))
}

/// GET /api/v1/search/index/segments/doc-bytes-ratio-by-segment — razão num_docs/disk_bytes por segmento. Sprint #5152.
pub async fn segment_doc_bytes_ratio_by_segment(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    let rows: Vec<serde_json::Value> = segs.iter()
        .map(|(id, nd, db)| {
            let ratio = if *db > 0 { *nd as f64 / *db as f64 } else { 0.0 };
            serde_json::json!({"segment_id": id, "num_docs": nd, "disk_bytes": db, "docs_per_byte": ratio})
        })
        .collect();
    Json(serde_json::json!({"rows": rows, "segment_count": n}))
}
...
pub async fn segment_docs_iqr_outliers(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    ...
    let mut docs: Vec<u64> = segs.iter().map(|(_, nd, _)| *nd).collect();
    docs.sort_unstable();
    let q1 = docs[(n * 25) / 100] as f64;
    let q3 = docs[(n * 75) / 100] as f64;
    let iqr = q3 - q1;
    let lo = q1 - 1.5 * iqr;
    let hi = q3 + 1.5 * iqr;
    let outliers: Vec<serde_json::Value> = segs.iter()
        .filter(|(_, nd, _)| (*nd as f64) < lo || (*nd as f64) > hi)
        .map(|(id, nd, db)| serde_json::json!({"segment_id": id, "num_docs": nd, "disk_bytes": db}))
        .collect();
    Json(serde_json::json!({"q1": q1, "q3": q3, "iqr": iqr, "fence_lo": lo, "fence_hi": hi, "outlier_count": outliers.len(), "segment_count": n, "outliers": outliers}))
}

/// GET /api/v1/search/index/segments/bytes-iqr-outliers — segmentos com disk_bytes fora de [Q1-1.5*IQR, Q3+1.5*IQR]. Sprint #5106.
pub async fn segment_bytes_iqr_outliers(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"q1": null, "q3": null, "iqr": null, "outliers": [], "segment_count": 0}));
...
/// GET /api/v1/search/index/segments/docs-zscore-above-1 — segmentos com z-score de num_docs > 1. Sprint #5107.
pub async fn segment_docs_zscore_above_1(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"mean": null, "stddev": null, "above": [], "segment_count": 0}));
    ...
    let stddev = (vals.iter().map(|v| (v - mean).powi(2)).sum::<f64>() / n as f64).sqrt();
    let above: Vec<serde_json::Value> = segs.iter()
        .filter(|(_, nd, _)| stddev > 0.0 && (*nd as f64 - mean) / stddev > 1.0)
        .map(|(id, nd, db)| serde_json::json!({"segment_id": id, "num_docs": nd, "disk_bytes": db}))
        .collect();
    Json(serde_json::json!({"mean_docs": mean, "stddev_docs": stddev, "above_count": above.len(), "segment_count": n, "segments": above}))
}

/// GET /api/v1/search/index/segments/bytes-zscore-above-1 — segmentos com z-score de disk_bytes > 1. Sprint #5108.
pub async fn segment_bytes_zscore_above_1(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"mean": null, "stddev": null, "above": [], "segment_count": 0}));
    ...
    let above: Vec<serde_json::Value> = segs.iter()
        ...
        .map(|(id, nd, db)| serde_json::json!({"segment_id": id, "num_docs": nd, "disk_bytes": db}))
        .collect();
    Json(serde_json::json!({"mean_bytes": mean, "stddev_bytes": stddev, "above_count": above.len(), "segment_count": n, "segments": above}))
}

/// GET /api/v1/search/index/segments/docs-zscore-above-2 — segmentos com z-score de num_docs > 2. Sprint #5109.
pub async fn segment_docs_zscore_above_2(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"mean": null, "stddev": null, "above": [], "segment_count": 0}));
...
/// GET /api/v1/search/index/segments/bytes-zscore-above-2 — segmentos com z-score de disk_bytes > 2. Sprint #5110.
pub async fn segment_bytes_zscore_above_2(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"mean": null, "stddev": null, "above": [], "segment_count": 0}));
...
/// GET /api/v1/search/index/segments/docs-zscore-below-1 — segmentos com z-score de num_docs < -1. Sprint #5111.
pub async fn segment_docs_zscore_below_1(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"mean": null, "stddev": null, "below": [], "segment_count": 0}));
...
/// GET /api/v1/search/index/segments/bytes-zscore-below-1 — segmentos com z-score de disk_bytes < -1. Sprint #5112.
pub async fn segment_bytes_zscore_below_1(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"mean": null, "stddev": null, "below": [], "segment_count": 0}));
...
pub async fn segment_bytes_per_doc_p01(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p01_bytes_per_doc": null, "total_segments": 0})); }
    let mut vals: Vec<f64> = segs.iter().filter_map(|(_, d, b)| if *d > 0 { Some(*b as f64 / *d as f64) } else { None }).collect();
    vals.sort_unstable_by(|a, b| a.partial_cmp(b).unwrap());
    let m = vals.len();
    let p01 = vals[(m * 1).saturating_sub(1).min(m - 1) / 100];
    Json(serde_json::json!({"p01_bytes_per_doc": p01, "total_segments": n}))
}

pub async fn segment_bytes_per_doc_p05(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p05_bytes_per_doc": null, "total_segments": 0})); }
    let mut vals: Vec<f64> = segs.iter().filter_map(|(_, d, b)| if *d > 0 { Some(*b as f64 / *d as f64) } else { None }).collect();
    vals.sort_unstable_by(|a, b| a.partial_cmp(b).unwrap());
    let m = vals.len();
    let p05 = vals[((m * 5) / 100).min(m - 1)];
    Json(serde_json::json!({"p05_bytes_per_doc": p05, "total_segments": n}))
}

pub async fn segment_bytes_per_doc_above_p01(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p01_bytes_per_doc": null, "above_count": 0, "total_segments": 0})); }
    let mut vals: Vec<f64> = segs.iter().filter_map(|(_, d, b)| if *d > 0 { Some(*b as f64 / *d as f64) } else { None }).collect();
    vals.sort_unstable_by(|a, b| a.partial_cmp(b).unwrap());
    let m = vals.len();
    let p01 = vals[((m * 1) / 100).min(m - 1)];
    let above = vals.iter().filter(|&&v| v > p01).count();
    Json(serde_json::json!({"p01_bytes_per_doc": p01, "above_count": above, "above_ratio": above as f64 / n as f64, "total_segments": n}))
}

pub async fn segment_bytes_per_doc_above_p05(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p05_bytes_per_doc": null, "above_count": 0, "total_segments": 0})); }
    let mut vals: Vec<f64> = segs.iter().filter_map(|(_, d, b)| if *d > 0 { Some(*b as f64 / *d as f64) } else { None }).collect();
    vals.sort_unstable_by(|a, b| a.partial_cmp(b).unwrap());
    let m = vals.len();
    let p05 = vals[((m * 5) / 100).min(m - 1)];
    let above = vals.iter().filter(|&&v| v > p05).count();
    Json(serde_json::json!({"p05_bytes_per_doc": p05, "above_count": above, "above_ratio": above as f64 / n as f64, "total_segments": n}))
}

pub async fn segment_bytes_per_doc_above_p10(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p10_bytes_per_doc": null, "above_count": 0, "total_segments": 0})); }
    let mut vals: Vec<f64> = segs.iter().filter_map(|(_, d, b)| if *d > 0 { Some(*b as f64 / *d as f64) } else { None }).collect();
    vals.sort_unstable_by(|a, b| a.partial_cmp(b).unwrap());
    let m = vals.len();
    let p10 = vals[((m * 10) / 100).min(m - 1)];
    let above = vals.iter().filter(|&&v| v > p10).count();
    Json(serde_json::json!({"p10_bytes_per_doc": p10, "above_count": above, "above_ratio": above as f64 / n as f64, "total_segments": n}))
}

pub async fn segment_bytes_per_doc_below_p01(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p01_bytes_per_doc": null, "below_count": 0, "total_segments": 0})); }
    let mut vals: Vec<f64> = segs.iter().filter_map(|(_, d, b)| if *d > 0 { Some(*b as f64 / *d as f64) } else { None }).collect();
    vals.sort_unstable_by(|a, b| a.partial_cmp(b).unwrap());
    let m = vals.len();
    let p01 = vals[((m * 1) / 100).min(m - 1)];
    let below = vals.iter().filter(|&&v| v < p01).count();
    Json(serde_json::json!({"p01_bytes_per_doc": p01, "below_count": below, "below_ratio": below as f64 / n as f64, "total_segments": n}))
}

pub async fn segment_bytes_per_doc_below_p05(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p05_bytes_per_doc": null, "below_count": 0, "total_segments": 0})); }
    let mut vals: Vec<f64> = segs.iter().filter_map(|(_, d, b)| if *d > 0 { Some(*b as f64 / *d as f64) } else { None }).collect();
    vals.sort_unstable_by(|a, b| a.partial_cmp(b).unwrap());
    let m = vals.len();
    let p05 = vals[((m * 5) / 100).min(m - 1)];
    let below = vals.iter().filter(|&&v| v < p05).count();
    Json(serde_json::json!({"p05_bytes_per_doc": p05, "below_count": below, "below_ratio": below as f64 / n as f64, "total_segments": n}))
}

pub async fn segment_bytes_per_doc_below_p10(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p10_bytes_per_doc": null, "below_count": 0, "total_segments": 0})); }
    let mut vals: Vec<f64> = segs.iter().filter_map(|(_, d, b)| if *d > 0 { Some(*b as f64 / *d as f64) } else { None }).collect();
    vals.sort_unstable_by(|a, b| a.partial_cmp(b).unwrap());
    let m = vals.len();
    let p10 = vals[((m * 10) / 100).min(m - 1)];
    let below = vals.iter().filter(|&&v| v < p10).count();
    Json(serde_json::json!({"p10_bytes_per_doc": p10, "below_count": below, "below_ratio": below as f64 / n as f64, "total_segments": n}))
}

pub async fn segment_bytes_per_doc_count_above_mean(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"mean_bytes_per_doc": null, "above_count": 0, "total_segments": 0})); }
    let vals: Vec<f64> = segs.iter().filter_map(|(_, d, b)| if *d > 0 { Some(*b as f64 / *d as f64) } else { None }).collect();
    let m = vals.len();
    let mean = vals.iter().sum::<f64>() / m as f64;
    let above = vals.iter().filter(|&&v| v > mean).count();
    Json(serde_json::json!({"mean_bytes_per_doc": mean, "above_count": above, "above_ratio": above as f64 / n as f64, "total_segments": n}))
}

pub async fn segment_bytes_per_doc_count_below_mean(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"mean_bytes_per_doc": null, "below_count": 0, "total_segments": 0})); }
    let vals: Vec<f64> = segs.iter().filter_map(|(_, d, b)| if *d > 0 { Some(*b as f64 / *d as f64) } else { None }).collect();
    let m = vals.len();
    let mean = vals.iter().sum::<f64>() / m as f64;
    let below = vals.iter().filter(|&&v| v < mean).count();
    Json(serde_json::json!({"mean_bytes_per_doc": mean, "below_count": below, "below_ratio": below as f64 / n as f64, "total_segments": n}))
}

pub async fn segment_bytes_per_doc_count_above_p01(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p01_bytes_per_doc": null, "above_count": 0, "total_segments": 0})); }
    let mut vals: Vec<f64> = segs.iter().filter_map(|(_, d, b)| if *d > 0 { Some(*b as f64 / *d as f64) } else { None }).collect();
...
pub async fn segment_bytes_per_doc_count_below_p01(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p01_bytes_per_doc": null, "below_count": 0, "total_segments": 0})); }
    let mut vals: Vec<f64> = segs.iter().filter_map(|(_, d, b)| if *d > 0 { Some(*b as f64 / *d as f64) } else { None }).collect();
...
pub async fn segment_bytes_per_doc_count_above_p05(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p05_bytes_per_doc": null, "above_count": 0, "total_segments": 0})); }
    let mut vals: Vec<f64> = segs.iter().filter_map(|(_, d, b)| if *d > 0 { Some(*b as f64 / *d as f64) } else { None }).collect();
...
pub async fn segment_bytes_per_doc_count_below_p05(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p05_bytes_per_doc": null, "below_count": 0, "total_segments": 0})); }
    let mut vals: Vec<f64> = segs.iter().filter_map(|(_, d, b)| if *d > 0 { Some(*b as f64 / *d as f64) } else { None }).collect();
...
pub async fn segment_bytes_per_doc_count_above_p10(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p10_bytes_per_doc": null, "above_count": 0, "total_segments": 0})); }
    let mut vals: Vec<f64> = segs.iter().filter_map(|(_, d, b)| if *d > 0 { Some(*b as f64 / *d as f64) } else { None }).collect();
...
pub async fn segment_bytes_per_doc_count_below_p10(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p10_bytes_per_doc": null, "below_count": 0, "total_segments": 0})); }
    let mut vals: Vec<f64> = segs.iter().filter_map(|(_, d, b)| if *d > 0 { Some(*b as f64 / *d as f64) } else { None }).collect();
...
pub async fn segment_bytes_per_doc_count_above_p25(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p25_bytes_per_doc": null, "above_count": 0, "total_segments": 0})); }
    let mut vals: Vec<f64> = segs.iter().filter_map(|(_, d, b)| if *d > 0 { Some(*b as f64 / *d as f64) } else { None }).collect();
    vals.sort_unstable_by(|a, b| a.partial_cmp(b).unwrap());
    let m = vals.len();
    let p25 = vals[((m * 25) / 100).min(m - 1)];
    let above = vals.iter().filter(|&&v| v > p25).count();
    Json(serde_json::json!({"p25_bytes_per_doc": p25, "above_count": above, "above_ratio": above as f64 / n as f64, "total_segments": n}))
}

pub async fn segment_bytes_per_doc_count_below_p25(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p25_bytes_per_doc": null, "below_count": 0, "total_segments": 0})); }
    let mut vals: Vec<f64> = segs.iter().filter_map(|(_, d, b)| if *d > 0 { Some(*b as f64 / *d as f64) } else { None }).collect();
    vals.sort_unstable_by(|a, b| a.partial_cmp(b).unwrap());
    let m = vals.len();
    let p25 = vals[((m * 25) / 100).min(m - 1)];
    let below = vals.iter().filter(|&&v| v < p25).count();
    Json(serde_json::json!({"p25_bytes_per_doc": p25, "below_count": below, "below_ratio": below as f64 / n as f64, "total_segments": n}))
}

pub async fn segment_bytes_per_doc_count_above_p50(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p50_bytes_per_doc": null, "above_count": 0, "total_segments": 0})); }
    let mut vals: Vec<f64> = segs.iter().filter_map(|(_, d, b)| if *d > 0 { Some(*b as f64 / *d as f64) } else { None }).collect();
    vals.sort_unstable_by(|a, b| a.partial_cmp(b).unwrap());
    let m = vals.len();
    let p50 = vals[((m * 50) / 100).min(m - 1)];
    let above = vals.iter().filter(|&&v| v > p50).count();
    Json(serde_json::json!({"p50_bytes_per_doc": p50, "above_count": above, "above_ratio": above as f64 / n as f64, "total_segments": n}))
}

pub async fn segment_bytes_per_doc_count_below_p50(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p50_bytes_per_doc": null, "below_count": 0, "total_segments": 0})); }
    let mut vals: Vec<f64> = segs.iter().filter_map(|(_, d, b)| if *d > 0 { Some(*b as f64 / *d as f64) } else { None }).collect();
    vals.sort_unstable_by(|a, b| a.partial_cmp(b).unwrap());
    let m = vals.len();
    let p50 = vals[((m * 50) / 100).min(m - 1)];
    let below = vals.iter().filter(|&&v| v < p50).count();
    Json(serde_json::json!({"p50_bytes_per_doc": p50, "below_count": below, "below_ratio": below as f64 / n as f64, "total_segments": n}))
}

pub async fn segment_bytes_per_doc_count_above_p75(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p75_bytes_per_doc": null, "above_count": 0, "total_segments": 0})); }
    let mut vals: Vec<f64> = segs.iter().filter_map(|(_, d, b)| if *d > 0 { Some(*b as f64 / *d as f64) } else { None }).collect();
    vals.sort_unstable_by(|a, b| a.partial_cmp(b).unwrap());
    let m = vals.len();
    let p75 = vals[((m * 75) / 100).min(m - 1)];
    let above = vals.iter().filter(|&&v| v > p75).count();
    Json(serde_json::json!({"p75_bytes_per_doc": p75, "above_count": above, "above_ratio": above as f64 / n as f64, "total_segments": n}))
}

pub async fn segment_bytes_per_doc_count_below_p75(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p75_bytes_per_doc": null, "below_count": 0, "total_segments": 0})); }
    let mut vals: Vec<f64> = segs.iter().filter_map(|(_, d, b)| if *d > 0 { Some(*b as f64 / *d as f64) } else { None }).collect();
    vals.sort_unstable_by(|a, b| a.partial_cmp(b).unwrap());
    let m = vals.len();
    let p75 = vals[((m * 75) / 100).min(m - 1)];
    let below = vals.iter().filter(|&&v| v < p75).count();
    Json(serde_json::json!({"p75_bytes_per_doc": p75, "below_count": below, "below_ratio": below as f64 / n as f64, "total_segments": n}))
}

pub async fn segment_bytes_per_doc_count_above_p90(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p90_bytes_per_doc": null, "above_count": 0, "total_segments": 0})); }
    let mut vals: Vec<f64> = segs.iter().filter_map(|(_, d, b)| if *d > 0 { Some(*b as f64 / *d as f64) } else { None }).collect();
    vals.sort_unstable_by(|a, b| a.partial_cmp(b).unwrap());
    let m = vals.len();
    let p90 = vals[((m * 90) / 100).min(m - 1)];
    let above = vals.iter().filter(|&&v| v > p90).count();
    Json(serde_json::json!({"p90_bytes_per_doc": p90, "above_count": above, "above_ratio": above as f64 / n as f64, "total_segments": n}))
}

pub async fn segment_bytes_per_doc_count_below_p90(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p90_bytes_per_doc": null, "below_count": 0, "total_segments": 0})); }
    let mut vals: Vec<f64> = segs.iter().filter_map(|(_, d, b)| if *d > 0 { Some(*b as f64 / *d as f64) } else { None }).collect();
    vals.sort_unstable_by(|a, b| a.partial_cmp(b).unwrap());
    let m = vals.len();
    let p90 = vals[((m * 90) / 100).min(m - 1)];
    let below = vals.iter().filter(|&&v| v < p90).count();
...
pub async fn segment_bytes_sum_below_p99(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    ...
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"sum_bytes_below_p99": 0, "total_segments": 0}));
    }
    ...
    let sum: u64 = vals.iter().filter(|&&v| v < p99).sum();
    let count = vals.iter().filter(|&&v| v < p99).count();
    Json(serde_json::json!({"sum_bytes_below_p99": sum, "p99_bytes": p99, "count_below_p99": count, "total_segments": n}))
}

/// GET /api/v1/search/index/segments/name-length-sum-above-p10 — soma de comprimento de nome dos segmentos acima do P10. Sprint #4738.
pub async fn segment_name_length_sum_above_p10(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"sum_name_length_above_p10": 0, "total_segments": 0}));
    }
    let mut vals: Vec<u64> = segs.iter().map(|(name, _, _)| name.len() as u64).collect();
    vals.sort_unstable();
    ...
    Json(serde_json::json!({"sum_name_length_above_p10": sum, "p10_name_length": p10, "count_above_p10": count, "total_segments": n}))
}

/// GET /api/v1/search/index/segments/name-length-sum-above-p05 — soma de comprimento de nome dos segmentos acima do P05. Sprint #4739.
pub async fn segment_name_length_sum_above_p05(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"sum_name_length_above_p05": 0, "total_segments": 0}));
    }
    let mut vals: Vec<u64> = segs.iter().map(|(name, _, _)| name.len() as u64).collect();
    vals.sort_unstable();
    ...
    Json(serde_json::json!({"sum_name_length_above_p05": sum, "p05_name_length": p05, "count_above_p05": count, "total_segments": n}))
}

/// GET /api/v1/search/index/segments/name-length-sum-below-p90 — soma de comprimento de nome dos segmentos abaixo do P90. Sprint #4740.
pub async fn segment_name_length_sum_below_p90(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"sum_name_length_below_p90": 0, "total_segments": 0}));
    }
    let mut vals: Vec<u64> = segs.iter().map(|(name, _, _)| name.len() as u64).collect();
    vals.sort_unstable();
    ...
    Json(serde_json::json!({"sum_name_length_below_p90": sum, "p90_name_length": p90, "count_below_p90": count, "total_segments": n}))
}

/// GET /api/v1/search/index/segments/name-length-sum-above-p01 — soma de comprimento de nome dos segmentos acima do P01. Sprint #4717.
pub async fn segment_name_length_sum_above_p01(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"sum_name_length_above_p01": 0, "total_segments": 0}));
    }
    let mut vals: Vec<u64> = segs.iter().map(|(name, _, _)| name.len() as u64).collect();
    vals.sort_unstable();
    ...
    Json(serde_json::json!({"sum_name_length_above_p01": sum, "p01_name_length": p01, "count_above_p01": count, "total_segments": n}))
}

/// GET /api/v1/search/index/segments/bytes-sum-below-p75 — soma de bytes dos segmentos abaixo do P75. Sprint #4718.
pub async fn segment_bytes_sum_below_p75(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"sum_bytes_below_p75": 0, "total_segments": 0}));
    }
    let mut vals: Vec<u64> = segs.iter().map(|(_, bytes, _)| *bytes).collect();
    vals.sort_unstable();
    let p75_idx = ((n as f64 * 0.75).ceil() as usize).min(n - 1);
    ...
    let count = vals.iter().filter(|&&v| v < p75).count();
    Json(serde_json::json!({"sum_bytes_below_p75": sum, "p75_bytes": p75, "count_below_p75": count, "total_segments": n}))
}

/// GET /api/v1/search/index/segments/bytes-sum-below-p90 — soma de bytes dos segmentos abaixo do P90. Sprint #4719.
pub async fn segment_bytes_sum_below_p90(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"sum_bytes_below_p90": 0, "total_segments": 0}));
    }
    let mut vals: Vec<u64> = segs.iter().map(|(_, bytes, _)| *bytes).collect();
    vals.sort_unstable();
    let p90_idx = ((n as f64 * 0.90).ceil() as usize).min(n - 1);
    ...
    let sum: u64 = vals.iter().filter(|&&v| v < p90).sum();
    let count = vals.iter().filter(|&&v| v < p90).count();
    Json(serde_json::json!({"sum_bytes_below_p90": sum, "p90_bytes": p90, "count_below_p90": count, "total_segments": n}))
}

/// GET /api/v1/search/index/segments/bytes-sum-below-p95 — soma de bytes dos segmentos abaixo do P95. Sprint #4720.
pub async fn segment_bytes_sum_below_p95(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"sum_bytes_below_p95": 0, "total_segments": 0}));
    }
    let mut vals: Vec<u64> = segs.iter().map(|(_, bytes, _)| *bytes).collect();
    vals.sort_unstable();
    let p95_idx = ((n as f64 * 0.95).ceil() as usize).min(n - 1);
    ...
    let sum: u64 = vals.iter().filter(|&&v| v < p95).sum();
    let count = vals.iter().filter(|&&v| v < p95).count();
    Json(serde_json::json!({"sum_bytes_below_p95": sum, "p95_bytes": p95, "count_below_p95": count, "total_segments": n}))
}
...
pub async fn segment_bytes_sum_p01(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    ...
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"bytes_p01": null, "total_segments": 0}));
    }
    let mut vals: Vec<u64> = segs.iter().map(|(_, _, b)| *b).collect();
    vals.sort_unstable();
    let idx = ((n as f64 * 0.01).ceil() as usize).saturating_sub(1).min(n - 1);
    Json(serde_json::json!({"bytes_p01": vals[idx], "total_segments": n}))
}

/// GET /api/v1/search/index/segments/bytes-sum-p25 — P25 de bytes por segmento. Sprint #4838.
pub async fn segment_bytes_sum_p25(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"bytes_p25": null, "total_segments": 0}));
    }
    let mut vals: Vec<u64> = segs.iter().map(|(_, _, b)| *b).collect();
    vals.sort_unstable();
    let idx = ((n as f64 * 0.25).ceil() as usize).saturating_sub(1).min(n - 1);
    Json(serde_json::json!({"bytes_p25": vals[idx], "total_segments": n}))
}

/// GET /api/v1/search/index/segments/docs-sum-p01 — P01 de docs por segmento. Sprint #4839.
pub async fn segment_docs_sum_p01(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"docs_p01": null, "total_segments": 0}));
    }
    let mut vals: Vec<u64> = segs.iter().map(|(_, d, _)| *d).collect();
    vals.sort_unstable();
    let idx = ((n as f64 * 0.01).ceil() as usize).saturating_sub(1).min(n - 1);
    Json(serde_json::json!({"docs_p01": vals[idx], "total_segments": n}))
}

/// GET /api/v1/search/index/segments/docs-sum-p25 — P25 de docs por segmento. Sprint #4840.
pub async fn segment_docs_sum_p25(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"docs_p25": null, "total_segments": 0}));
    }
    let mut vals: Vec<u64> = segs.iter().map(|(_, d, _)| *d).collect();
    vals.sort_unstable();
    let idx = ((n as f64 * 0.25).ceil() as usize).saturating_sub(1).min(n - 1);
    Json(serde_json::json!({"docs_p25": vals[idx], "total_segments": n}))
}

/// GET /api/v1/search/index/segments/bytes-sum-p90 — P90 de bytes por segmento. Sprint #4897.
pub async fn segment_bytes_sum_p90(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"bytes_p90": null, "total_segments": 0}));
    }
    let mut vals: Vec<u64> = segs.iter().map(|(_, _, b)| *b).collect();
    vals.sort_unstable();
    let idx = ((n as f64 * 0.90).ceil() as usize).saturating_sub(1).min(n - 1);
    Json(serde_json::json!({"bytes_p90": vals[idx], "total_segments": n}))
}

/// GET /api/v1/search/index/segments/docs-sum-p90 — P90 de docs por segmento. Sprint #4898.
pub async fn segment_docs_sum_p90(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"docs_p90": null, "total_segments": 0}));
    }
    let mut vals: Vec<u64> = segs.iter().map(|(_, d, _)| *d).collect();
    vals.sort_unstable();
    let idx = ((n as f64 * 0.90).ceil() as usize).saturating_sub(1).min(n - 1);
    Json(serde_json::json!({"docs_p90": vals[idx], "total_segments": n}))
}

/// GET /api/v1/search/index/segments/name-length-p90 — P90 do comprimento de nome dos segmentos. Sprint #4899.
pub async fn segment_name_length_p90(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
...
/// GET /api/v1/search/index/segments/name-length-p75 — P75 do comprimento de nome dos segmentos. Sprint #4900.
pub async fn segment_name_length_p75(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
...
/// GET /api/v1/search/index/segments/bytes-sum-p50 — P50 de bytes por segmento. Sprint #4877.
pub async fn segment_bytes_sum_p50(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"bytes_p50": null, "total_segments": 0}));
    }
    let mut vals: Vec<u64> = segs.iter().map(|(_, _, b)| *b).collect();
    vals.sort_unstable();
    let idx = ((n as f64 * 0.50).ceil() as usize).saturating_sub(1).min(n - 1);
    Json(serde_json::json!({"bytes_p50": vals[idx], "total_segments": n}))
}

/// GET /api/v1/search/index/segments/bytes-sum-p75 — P75 de bytes por segmento. Sprint #4878.
pub async fn segment_bytes_sum_p75(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"bytes_p75": null, "total_segments": 0}));
    }
    let mut vals: Vec<u64> = segs.iter().map(|(_, _, b)| *b).collect();
    vals.sort_unstable();
    let idx = ((n as f64 * 0.75).ceil() as usize).saturating_sub(1).min(n - 1);
    Json(serde_json::json!({"bytes_p75": vals[idx], "total_segments": n}))
}

/// GET /api/v1/search/index/segments/docs-sum-p50 — P50 de docs por segmento. Sprint #4879.
pub async fn segment_docs_sum_p50(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"docs_p50": null, "total_segments": 0}));
    }
    let mut vals: Vec<u64> = segs.iter().map(|(_, d, _)| *d).collect();
    vals.sort_unstable();
    let idx = ((n as f64 * 0.50).ceil() as usize).saturating_sub(1).min(n - 1);
    Json(serde_json::json!({"docs_p50": vals[idx], "total_segments": n}))
}

/// GET /api/v1/search/index/segments/docs-sum-p75 — P75 de docs por segmento. Sprint #4880.
pub async fn segment_docs_sum_p75(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"docs_p75": null, "total_segments": 0}));
    }
    let mut vals: Vec<u64> = segs.iter().map(|(_, d, _)| *d).collect();
    vals.sort_unstable();
    let idx = ((n as f64 * 0.75).ceil() as usize).saturating_sub(1).min(n - 1);
    Json(serde_json::json!({"docs_p75": vals[idx], "total_segments": n}))
}

/// GET /api/v1/search/index/segments/bytes-sum-p95 — P95 de bytes por segmento. Sprint #4817.
pub async fn segment_bytes_sum_p95(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"bytes_p95": null, "total_segments": 0}));
    ...
    vals.sort_unstable();
    let idx = ((n as f64 * 0.95).ceil() as usize).saturating_sub(1).min(n - 1);
    Json(serde_json::json!({"bytes_p95": vals[idx], "total_segments": n}))
}

/// GET /api/v1/search/index/segments/bytes-sum-p99 — P99 de bytes por segmento. Sprint #4818.
pub async fn segment_bytes_sum_p99(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"bytes_p99": null, "total_segments": 0}));
    }
    let mut vals: Vec<u64> = segs.iter().map(|(_, _, b)| *b).collect();
    vals.sort_unstable();
    let idx = ((n as f64 * 0.99).ceil() as usize).saturating_sub(1).min(n - 1);
    Json(serde_json::json!({"bytes_p99": vals[idx], "total_segments": n}))
}

/// GET /api/v1/search/index/segments/docs-sum-p95 — P95 de docs por segmento. Sprint #4819.
pub async fn segment_docs_sum_p95(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"docs_p95": null, "total_segments": 0}));
    }
    let mut vals: Vec<u64> = segs.iter().map(|(_, d, _)| *d).collect();
    vals.sort_unstable();
    let idx = ((n as f64 * 0.95).ceil() as usize).saturating_sub(1).min(n - 1);
    Json(serde_json::json!({"docs_p95": vals[idx], "total_segments": n}))
}

/// GET /api/v1/search/index/segments/docs-sum-p99 — P99 de docs por segmento. Sprint #4820.
pub async fn segment_docs_sum_p99(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"docs_p99": null, "total_segments": 0}));
    }
    let mut vals: Vec<u64> = segs.iter().map(|(_, d, _)| *d).collect();
    vals.sort_unstable();
    let idx = ((n as f64 * 0.99).ceil() as usize).saturating_sub(1).min(n - 1);
    Json(serde_json::json!({"docs_p99": vals[idx], "total_segments": n}))
}

/// GET /api/v1/search/index/segments/name-length-count-above-p95 — contagem de segmentos com comprimento de nome acima do P95. Sprint #4797.
pub async fn segment_name_length_count_above_p95(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"count_above_p95": 0, "total_segments": 0}));
    }
    let mut vals: Vec<usize> = segs.iter().map(|(name, _, _)| name.len()).collect();
    vals.sort_unstable();
    let idx = ((n as f64 * 0.95).ceil() as usize).saturating_sub(1).min(n - 1);
    let p95 = vals[idx];
    let count = segs.iter().filter(|(name, _, _)| name.len() > p95).count();
    Json(serde_json::json!({"count_above_p95": count, "p95_name_length": p95, "total_segments": n}))
}

/// GET /api/v1/search/index/segments/name-length-count-below-p95 — contagem de segmentos com comprimento de nome abaixo do P95. Sprint #4798.
pub async fn segment_name_length_count_below_p95(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"count_below_p95": 0, "total_segments": 0}));
    }
    let mut vals: Vec<usize> = segs.iter().map(|(name, _, _)| name.len()).collect();
    vals.sort_unstable();
    let idx = ((n as f64 * 0.95).ceil() as usize).saturating_sub(1).min(n - 1);
    let p95 = vals[idx];
    let count = segs.iter().filter(|(name, _, _)| name.len() < p95).count();
    Json(serde_json::json!({"count_below_p95": count, "p95_name_length": p95, "total_segments": n}))
}

/// GET /api/v1/search/index/segments/name-length-count-above-p99 — contagem de segmentos com comprimento de nome acima do P99. Sprint #4799.
pub async fn segment_name_length_count_above_p99(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"count_above_p99": 0, "total_segments": 0}));
    }
    let mut vals: Vec<usize> = segs.iter().map(|(name, _, _)| name.len()).collect();
    vals.sort_unstable();
    let idx = ((n as f64 * 0.99).ceil() as usize).saturating_sub(1).min(n - 1);
    let p99 = vals[idx];
    let count = segs.iter().filter(|(name, _, _)| name.len() > p99).count();
    Json(serde_json::json!({"count_above_p99": count, "p99_name_length": p99, "total_segments": n}))
}

/// GET /api/v1/search/index/segments/name-length-count-below-p99 — contagem de segmentos com comprimento de nome abaixo do P99. Sprint #4800.
pub async fn segment_name_length_count_below_p99(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"count_below_p99": 0, "total_segments": 0}));
    }
    let mut vals: Vec<usize> = segs.iter().map(|(name, _, _)| name.len()).collect();
    vals.sort_unstable();
    let idx = ((n as f64 * 0.99).ceil() as usize).saturating_sub(1).min(n - 1);
    let p99 = vals[idx];
    let count = segs.iter().filter(|(name, _, _)| name.len() < p99).count();
    Json(serde_json::json!({"count_below_p99": count, "p99_name_length": p99, "total_segments": n}))
}
...
/// GET /api/v1/search/index/segments/size-count-above-p90 — contagem de segmentos com tamanho acima do P90. Sprint #4977.
pub async fn segment_size_count_above_p90(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"count_above_p90": 0, "total_segments": 0}));
    ...
    let p90 = vals[idx];
    let count = vals.iter().filter(|&&v| v > p90).count();
    Json(serde_json::json!({"count_above_p90": count, "p90_size": p90, "total_segments": n}))
}

/// GET /api/v1/search/index/segments/size-count-below-p90 — contagem de segmentos com tamanho abaixo do P90. Sprint #4978.
pub async fn segment_size_count_below_p90(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"count_below_p90": 0, "total_segments": 0}));
    ...
    let p90 = vals[idx];
    let count = vals.iter().filter(|&&v| v < p90).count();
    Json(serde_json::json!({"count_below_p90": count, "p90_size": p90, "total_segments": n}))
}

/// GET /api/v1/search/index/segments/size-count-above-p95 — contagem de segmentos com tamanho acima do P95. Sprint #4979.
pub async fn segment_size_count_above_p95(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"count_above_p95": 0, "total_segments": 0}));
    ...
    let p95 = vals[idx];
    let count = vals.iter().filter(|&&v| v > p95).count();
    Json(serde_json::json!({"count_above_p95": count, "p95_size": p95, "total_segments": n}))
}

/// GET /api/v1/search/index/segments/size-count-below-p95 — contagem de segmentos com tamanho abaixo do P95. Sprint #4980.
pub async fn segment_size_count_below_p95(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"count_below_p95": 0, "total_segments": 0}));
    ...
    let p95 = vals[idx];
    let count = vals.iter().filter(|&&v| v < p95).count();
    Json(serde_json::json!({"count_below_p95": count, "p95_size": p95, "total_segments": n}))
}

/// GET /api/v1/search/index/segments/size-count-above-p50 — contagem de segmentos com tamanho acima do P50. Sprint #4957.
pub async fn segment_size_count_above_p50(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"count_above_p50": 0, "total_segments": 0}));
    ...
    let p50 = vals[n / 2];
    let count = vals.iter().filter(|&&v| v > p50).count();
    Json(serde_json::json!({"count_above_p50": count, "p50_size": p50, "total_segments": n}))
}

/// GET /api/v1/search/index/segments/size-count-below-p50 — contagem de segmentos com tamanho abaixo do P50. Sprint #4958.
pub async fn segment_size_count_below_p50(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"count_below_p50": 0, "total_segments": 0}));
    ...
    let p50 = vals[n / 2];
    let count = vals.iter().filter(|&&v| v < p50).count();
    Json(serde_json::json!({"count_below_p50": count, "p50_size": p50, "total_segments": n}))
}

/// GET /api/v1/search/index/segments/size-count-above-p75 — contagem de segmentos com tamanho acima do P75. Sprint #4959.
pub async fn segment_size_count_above_p75(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"count_above_p75": 0, "total_segments": 0}));
    }
    let mut vals: Vec<u64> = segs.iter().map(|(_, _, b)| *b).collect();
    vals.sort_unstable();
    let idx = ((n as f64 * 0.75).ceil() as usize).saturating_sub(1).min(n - 1);
    let p75 = vals[idx];
    let count = vals.iter().filter(|&&v| v > p75).count();
    Json(serde_json::json!({"count_above_p75": count, "p75_size": p75, "total_segments": n}))
}

/// GET /api/v1/search/index/segments/size-count-below-p75 — contagem de segmentos com tamanho abaixo do P75. Sprint #4960.
pub async fn segment_size_count_below_p75(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"count_below_p75": 0, "total_segments": 0}));
    }
    let mut vals: Vec<u64> = segs.iter().map(|(_, _, b)| *b).collect();
    vals.sort_unstable();
    let idx = ((n as f64 * 0.75).ceil() as usize).saturating_sub(1).min(n - 1);
    let p75 = vals[idx];
    let count = vals.iter().filter(|&&v| v < p75).count();
    Json(serde_json::json!({"count_below_p75": count, "p75_size": p75, "total_segments": n}))
}

/// GET /api/v1/search/index/segments/size-count-above-p25 — contagem de segmentos com tamanho acima do P25. Sprint #5037.
pub async fn segment_size_count_above_p25(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"count_above_p25": 0, "total_segments": 0}));
    ...
    let p25 = vals[idx];
    let count = vals.iter().filter(|&&v| v > p25).count();
    Json(serde_json::json!({"count_above_p25": count, "p25_size": p25, "total_segments": n}))
}

/// GET /api/v1/search/index/segments/size-count-below-p25 — contagem de segmentos com tamanho abaixo do P25. Sprint #5038.
pub async fn segment_size_count_below_p25(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"count_below_p25": 0, "total_segments": 0}));
    ...
    let p25 = vals[idx];
    let count = vals.iter().filter(|&&v| v < p25).count();
    Json(serde_json::json!({"count_below_p25": count, "p25_size": p25, "total_segments": n}))
}

/// GET /api/v1/search/index/segments/size-count-above-p99 — contagem de segmentos com tamanho acima do P99. Sprint #5039.
pub async fn segment_size_count_above_p99(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"count_above_p99": 0, "total_segments": 0}));
    ...
    let p99 = vals[idx];
    let count = vals.iter().filter(|&&v| v > p99).count();
    Json(serde_json::json!({"count_above_p99": count, "p99_size": p99, "total_segments": n}))
}

/// GET /api/v1/search/index/segments/size-count-below-p99 — contagem de segmentos com tamanho abaixo do P99. Sprint #5040.
pub async fn segment_size_count_below_p99(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"count_below_p99": 0, "total_segments": 0}));
    ...
    let p99 = vals[idx];
    let count = vals.iter().filter(|&&v| v < p99).count();
    Json(serde_json::json!({"count_below_p99": count, "p99_size": p99, "total_segments": n}))
}

/// GET /api/v1/search/index/segments/size-count-above-p05 — contagem de segmentos com tamanho acima do P05. Sprint #5041.
pub async fn segment_size_count_above_p05(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"count_above_p05": 0, "total_segments": 0}));
    ...
    let p05 = vals[idx];
    let count = vals.iter().filter(|&&v| v > p05).count();
    Json(serde_json::json!({"count_above_p05": count, "p05_size": p05, "total_segments": n}))
}

/// GET /api/v1/search/index/segments/size-count-below-p05 — contagem de segmentos com tamanho abaixo do P05. Sprint #5042.
pub async fn segment_size_count_below_p05(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"count_below_p05": 0, "total_segments": 0}));
    ...
    let p05 = vals[idx];
    let count = vals.iter().filter(|&&v| v < p05).count();
    Json(serde_json::json!({"count_below_p05": count, "p05_size": p05, "total_segments": n}))
}

/// GET /api/v1/search/index/segments/size-count-above-p01 — contagem de segmentos com tamanho acima do P01. Sprint #5043.
pub async fn segment_size_count_above_p01(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"count_above_p01": 0, "total_segments": 0}));
    ...
    let p01 = vals[idx];
    let count = vals.iter().filter(|&&v| v > p01).count();
    Json(serde_json::json!({"count_above_p01": count, "p01_size": p01, "total_segments": n}))
}

/// GET /api/v1/search/index/segments/size-count-below-p01 — contagem de segmentos com tamanho abaixo do P01. Sprint #5044.
pub async fn segment_size_count_below_p01(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 {
        return Json(serde_json::json!({"count_below_p01": 0, "total_segments": 0}));
    ...
    let p01 = vals[idx];
    let count = vals.iter().filter(|&&v| v < p01).count();
    Json(serde_json::json!({"count_below_p01": count, "p01_size": p01, "total_segments": n}))
}
...
pub async fn segment_size_count_above_p10(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p10_size": null, "above_count": 0, "segment_count": 0})); }
    let mut sizes: Vec<u64> = segs.iter().map(|(_, _, db)| *db).collect();
    sizes.sort_unstable();
    let p10 = sizes[(n * 10) / 100];
    let above = segs.iter().filter(|(_, _, db)| *db > p10).count();
    Json(serde_json::json!({"p10_size": p10, "above_count": above, "above_ratio": above as f64 / n as f64, "segment_count": n}))
}

pub async fn segment_size_count_below_p10(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p10_size": null, "below_count": 0, "segment_count": 0})); }
    let mut sizes: Vec<u64> = segs.iter().map(|(_, _, db)| *db).collect();
    sizes.sort_unstable();
    let p10 = sizes[(n * 10) / 100];
    let below = segs.iter().filter(|(_, _, db)| *db < p10).count();
    Json(serde_json::json!({"p10_size": p10, "below_count": below, "below_ratio": below as f64 / n as f64, "segment_count": n}))
}

pub async fn segment_ratio_count_above_mean(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"mean_ratio": null, "above_count": 0, "segment_count": 0})); }
    let ratios: Vec<f64> = segs.iter().map(|(_, nd, db)| if *db > 0 { *nd as f64 / *db as f64 } else { 0.0 }).collect();
    let mean = ratios.iter().sum::<f64>() / n as f64;
    let above = ratios.iter().filter(|&&r| r > mean).count();
    Json(serde_json::json!({"mean_ratio": mean, "above_count": above, "above_ratio": above as f64 / n as f64, "segment_count": n}))
}

pub async fn segment_ratio_count_below_mean(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"mean_ratio": null, "below_count": 0, "segment_count": 0})); }
    let ratios: Vec<f64> = segs.iter().map(|(_, nd, db)| if *db > 0 { *nd as f64 / *db as f64 } else { 0.0 }).collect();
    let mean = ratios.iter().sum::<f64>() / n as f64;
    let below = ratios.iter().filter(|&&r| r < mean).count();
    Json(serde_json::json!({"mean_ratio": mean, "below_count": below, "below_ratio": below as f64 / n as f64, "segment_count": n}))
}

pub async fn segment_ratio_count_above_p25(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p25_ratio": null, "above_count": 0, "segment_count": 0})); }
    let mut ratios: Vec<f64> = segs.iter().map(|(_, nd, db)| if *db > 0 { *nd as f64 / *db as f64 } else { 0.0 }).collect();
    ratios.sort_unstable_by(|a, b| a.partial_cmp(b).unwrap());
    let p25 = ratios[(n * 25) / 100];
    let above = ratios.iter().filter(|&&r| r > p25).count();
    Json(serde_json::json!({"p25_ratio": p25, "above_count": above, "above_ratio": above as f64 / n as f64, "segment_count": n}))
}

pub async fn segment_ratio_count_below_p25(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p25_ratio": null, "below_count": 0, "segment_count": 0})); }
    let mut ratios: Vec<f64> = segs.iter().map(|(_, nd, db)| if *db > 0 { *nd as f64 / *db as f64 } else { 0.0 }).collect();
    ratios.sort_unstable_by(|a, b| a.partial_cmp(b).unwrap());
    let p25 = ratios[(n * 25) / 100];
    let below = ratios.iter().filter(|&&r| r < p25).count();
    Json(serde_json::json!({"p25_ratio": p25, "below_count": below, "below_ratio": below as f64 / n as f64, "segment_count": n}))
}

pub async fn segment_ratio_count_above_p10(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p10_ratio": null, "above_count": 0, "segment_count": 0})); }
    let mut ratios: Vec<f64> = segs.iter().map(|(_, nd, db)| if *db > 0 { *nd as f64 / *db as f64 } else { 0.0 }).collect();
    ratios.sort_unstable_by(|a, b| a.partial_cmp(b).unwrap());
    let p10 = ratios[(n * 10) / 100];
    let above = ratios.iter().filter(|&&r| r > p10).count();
    Json(serde_json::json!({"p10_ratio": p10, "above_count": above, "above_ratio": above as f64 / n as f64, "segment_count": n}))
}

pub async fn segment_ratio_count_below_p10(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p10_ratio": null, "below_count": 0, "segment_count": 0})); }
    let mut ratios: Vec<f64> = segs.iter().map(|(_, nd, db)| if *db > 0 { *nd as f64 / *db as f64 } else { 0.0 }).collect();
    ratios.sort_unstable_by(|a, b| a.partial_cmp(b).unwrap());
    let p10 = ratios[(n * 10) / 100];
    let below = ratios.iter().filter(|&&r| r < p10).count();
    Json(serde_json::json!({"p10_ratio": p10, "below_count": below, "below_ratio": below as f64 / n as f64, "segment_count": n}))
}

pub async fn segment_ratio_count_above_p50(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p50_ratio": null, "above_count": 0, "segment_count": 0})); }
    let mut ratios: Vec<f64> = segs.iter().map(|(_, nd, db)| if *db > 0 { *nd as f64 / *db as f64 } else { 0.0 }).collect();
    ratios.sort_unstable_by(|a, b| a.partial_cmp(b).unwrap());
    let p50 = ratios[(n * 50) / 100];
    let above = ratios.iter().filter(|&&r| r > p50).count();
    Json(serde_json::json!({"p50_ratio": p50, "above_count": above, "above_ratio": above as f64 / n as f64, "segment_count": n}))
}

pub async fn segment_ratio_count_below_p50(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p50_ratio": null, "below_count": 0, "segment_count": 0})); }
    let mut ratios: Vec<f64> = segs.iter().map(|(_, nd, db)| if *db > 0 { *nd as f64 / *db as f64 } else { 0.0 }).collect();
    ratios.sort_unstable_by(|a, b| a.partial_cmp(b).unwrap());
    let p50 = ratios[(n * 50) / 100];
    let below = ratios.iter().filter(|&&r| r < p50).count();
    Json(serde_json::json!({"p50_ratio": p50, "below_count": below, "below_ratio": below as f64 / n as f64, "segment_count": n}))
}

pub async fn segment_ratio_count_above_p75(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p75_ratio": null, "above_count": 0, "segment_count": 0})); }
    let mut ratios: Vec<f64> = segs.iter().map(|(_, nd, db)| if *db > 0 { *nd as f64 / *db as f64 } else { 0.0 }).collect();
    ratios.sort_unstable_by(|a, b| a.partial_cmp(b).unwrap());
    let p75 = ratios[(n * 75) / 100];
    let above = ratios.iter().filter(|&&r| r > p75).count();
    Json(serde_json::json!({"p75_ratio": p75, "above_count": above, "above_ratio": above as f64 / n as f64, "segment_count": n}))
}

pub async fn segment_ratio_count_below_p75(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p75_ratio": null, "below_count": 0, "segment_count": 0})); }
    let mut ratios: Vec<f64> = segs.iter().map(|(_, nd, db)| if *db > 0 { *nd as f64 / *db as f64 } else { 0.0 }).collect();
    ratios.sort_unstable_by(|a, b| a.partial_cmp(b).unwrap());
    let p75 = ratios[(n * 75) / 100];
    let below = ratios.iter().filter(|&&r| r < p75).count();
    Json(serde_json::json!({"p75_ratio": p75, "below_count": below, "below_ratio": below as f64 / n as f64, "segment_count": n}))
}

pub async fn segment_ratio_count_above_p90(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p90_ratio": null, "above_count": 0, "segment_count": 0})); }
    let mut ratios: Vec<f64> = segs.iter().map(|(_, nd, db)| if *db > 0 { *nd as f64 / *db as f64 } else { 0.0 }).collect();
    ratios.sort_unstable_by(|a, b| a.partial_cmp(b).unwrap());
    let p90 = ratios[(n * 90) / 100];
    let above = ratios.iter().filter(|&&r| r > p90).count();
    Json(serde_json::json!({"p90_ratio": p90, "above_count": above, "above_ratio": above as f64 / n as f64, "segment_count": n}))
}

pub async fn segment_ratio_count_below_p90(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p90_ratio": null, "below_count": 0, "segment_count": 0})); }
    let mut ratios: Vec<f64> = segs.iter().map(|(_, nd, db)| if *db > 0 { *nd as f64 / *db as f64 } else { 0.0 }).collect();
    ratios.sort_unstable_by(|a, b| a.partial_cmp(b).unwrap());
    let p90 = ratios[(n * 90) / 100];
    let below = ratios.iter().filter(|&&r| r < p90).count();
    Json(serde_json::json!({"p90_ratio": p90, "below_count": below, "below_ratio": below as f64 / n as f64, "segment_count": n}))
}

pub async fn segment_ratio_count_above_p95(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p95_ratio": null, "above_count": 0, "segment_count": 0})); }
    let mut ratios: Vec<f64> = segs.iter().map(|(_, nd, db)| if *db > 0 { *nd as f64 / *db as f64 } else { 0.0 }).collect();
    ratios.sort_unstable_by(|a, b| a.partial_cmp(b).unwrap());
    let p95 = ratios[(n * 95) / 100];
    let above = ratios.iter().filter(|&&r| r > p95).count();
    Json(serde_json::json!({"p95_ratio": p95, "above_count": above, "above_ratio": above as f64 / n as f64, "segment_count": n}))
}

pub async fn segment_ratio_count_below_p95(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p95_ratio": null, "below_count": 0, "segment_count": 0})); }
    let mut ratios: Vec<f64> = segs.iter().map(|(_, nd, db)| if *db > 0 { *nd as f64 / *db as f64 } else { 0.0 }).collect();
    ratios.sort_unstable_by(|a, b| a.partial_cmp(b).unwrap());
    let p95 = ratios[(n * 95) / 100];
    let below = ratios.iter().filter(|&&r| r < p95).count();
    Json(serde_json::json!({"p95_ratio": p95, "below_count": below, "below_ratio": below as f64 / n as f64, "segment_count": n}))
}

pub async fn segment_ratio_count_above_p99(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p99_ratio": null, "above_count": 0, "segment_count": 0})); }
    let mut ratios: Vec<f64> = segs.iter().map(|(_, nd, db)| if *db > 0 { *nd as f64 / *db as f64 } else { 0.0 }).collect();
    ratios.sort_unstable_by(|a, b| a.partial_cmp(b).unwrap());
    let p99 = ratios[(n * 99) / 100];
    let above = ratios.iter().filter(|&&r| r > p99).count();
    Json(serde_json::json!({"p99_ratio": p99, "above_count": above, "above_ratio": above as f64 / n as f64, "segment_count": n}))
}

pub async fn segment_ratio_count_below_p99(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p99_ratio": null, "below_count": 0, "segment_count": 0})); }
    let mut ratios: Vec<f64> = segs.iter().map(|(_, nd, db)| if *db > 0 { *nd as f64 / *db as f64 } else { 0.0 }).collect();
    ratios.sort_unstable_by(|a, b| a.partial_cmp(b).unwrap());
    let p99 = ratios[(n * 99) / 100];
    let below = ratios.iter().filter(|&&r| r < p99).count();
    Json(serde_json::json!({"p99_ratio": p99, "below_count": below, "below_ratio": below as f64 / n as f64, "segment_count": n}))
}
...
mod tests {
    ...
    fn empty_query_accepted() {
        let mut params = p("", DEFAULT_LIMIT);
        assert!(validate_search_params(&mut params).is_none());
    }

    #[test]
    fn offset_preserved_after_validation() {
...
pub async fn segment_ratio_count_above_p01(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p01_ratio": null, "above_count": 0, "segment_count": 0})); }
    let mut ratios: Vec<f64> = segs.iter().map(|(_, nd, db)| if *db > 0 { *nd as f64 / *db as f64 } else { 0.0 }).collect();
    ratios.sort_unstable_by(|a, b| a.partial_cmp(b).unwrap());
    let p01 = ratios[(n * 1) / 100];
    let above = ratios.iter().filter(|&&r| r > p01).count();
    Json(serde_json::json!({"p01_ratio": p01, "above_count": above, "above_ratio": above as f64 / n as f64, "segment_count": n}))
}

pub async fn segment_ratio_count_below_p01(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p01_ratio": null, "below_count": 0, "segment_count": 0})); }
    let mut ratios: Vec<f64> = segs.iter().map(|(_, nd, db)| if *db > 0 { *nd as f64 / *db as f64 } else { 0.0 }).collect();
    ratios.sort_unstable_by(|a, b| a.partial_cmp(b).unwrap());
    let p01 = ratios[(n * 1) / 100];
    let below = ratios.iter().filter(|&&r| r < p01).count();
    Json(serde_json::json!({"p01_ratio": p01, "below_count": below, "below_ratio": below as f64 / n as f64, "segment_count": n}))
}

pub async fn segment_ratio_count_above_p05(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p05_ratio": null, "above_count": 0, "segment_count": 0})); }
    let mut ratios: Vec<f64> = segs.iter().map(|(_, nd, db)| if *db > 0 { *nd as f64 / *db as f64 } else { 0.0 }).collect();
    ratios.sort_unstable_by(|a, b| a.partial_cmp(b).unwrap());
    let p05 = ratios[(n * 5) / 100];
    let above = ratios.iter().filter(|&&r| r > p05).count();
    Json(serde_json::json!({"p05_ratio": p05, "above_count": above, "above_ratio": above as f64 / n as f64, "segment_count": n}))
}

pub async fn segment_ratio_count_below_p05(State(store): State<IndexStore>) -> Json<serde_json::Value> {
    let segs = store.list_segments().unwrap_or_default();
    let n = segs.len();
    if n == 0 { return Json(serde_json::json!({"p05_ratio": null, "below_count": 0, "segment_count": 0})); }
    let mut ratios: Vec<f64> = segs.iter().map(|(_, nd, db)| if *db > 0 { *nd as f64 / *db as f64 } else { 0.0 }).collect();
    ratios.sort_unstable_by(|a, b| a.partial_cmp(b).unwrap());
    let p05 = ratios[(n * 5) / 100];
    let below = ratios.iter().filter(|&&r| r < p05).count();
    Json(serde_json::json!({"p05_ratio": p05, "below_count": below, "below_ratio": below as f64 / n as f64, "segment_count": n}))
}
