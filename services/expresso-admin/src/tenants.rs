...
pub async fn list(
    ...
    let rows = rows
        .into_iter()
        .map(|(id, slug, name, cnpj, plan, status, uc)| TenantRow {
            id: id.to_string(),
            slug,
            name,
...
fn validate(f: &TenantForm) -> Option<String> {
    ...
    if f.name.trim().is_empty() {
        return Some("nome obrigatório".into());
    }
    if !PLANS.contains(&f.plan.as_str()) {
        return Some("plano inválido".into());
    }
    if !STATUSES.contains(&f.status.as_str()) {
        return Some("status inválido".into());
    }
    None
}
...
pub async fn create_action(
    ...
    let pool = st
        ...
        .as_ref()
        .ok_or_else(|| AdminError(anyhow::anyhow!("database unavailable")))?;
    let cnpj = if f.cnpj.trim().is_empty() {
        None
    } else {
        Some(f.cnpj.trim().to_string())
    };
    let res = sqlx::query(
...
pub async fn delete_action(
    ...
    let actual_slug: Option<(String,)> = sqlx::query_as("SELECT slug FROM tenants WHERE id = $1")
        .bind(id)
        .fetch_optional(pool)
        .await
        .map_err(|e| AdminError(e.into()))?;
    ...
    if f.confirm_slug.trim() != slug {
        audit::record(
            ...
            &headers,
            &axum::http::Method::POST,
            &format!("/tenants/{id}/delete"),
            "admin.tenant.delete.rejected",
            Some("tenant"),
            Some(id.to_string()),
            Some(400),
...
pub async fn config_action(
    ...
    let parsed: serde_json::Value = match serde_json::from_str(&f.config_json) {
        Ok(v) => v,
        Err(e) => {
            return render_config_err(id, f.config_json, format!("JSON inválido: {e}"), pool).await
        }
    };
    if !parsed.is_object() {
        return render_config_err(
            id,
            f.config_json,
            "config precisa ser um JSON object".into(),
            pool,
        )
        .await;
    }
    ...
    audit::record(
        ...
        &headers,
        &axum::http::Method::POST,
        &format!("/tenants/{id}/config"),
        "admin.tenant.config_update",
        Some("tenant"),
        Some(id.to_string()),
        Some(302),
...
async fn render_config_err(
    ...
    let (slug, name) =
        sqlx::query_as::<_, (String, String)>(r#"SELECT slug, name FROM tenants WHERE id = $1"#)
            .bind(id)
            .fetch_one(pool)
            .await
            .map(|(s, n)| (s, n))
...
fn validate_wizard(f: &TenantWizardForm) -> Option<String> {
    ...
    if !f.admin_email.contains('@') {
        return Some("email admin inválido".into());
    }
    if f.admin_user.trim().is_empty() {
        return Some("username admin obrigatório".into());
    }
    if !PLANS.contains(&f.plan.as_str()) {
        return Some("plano inválido".into());
    }
...
pub async fn wizard_action(
    ...
    let render = |error: Option<String>, success: Option<String>, f: &TenantWizardForm| {
        crate::templates::TenantWizardTpl {
            ...
            name: f.name.clone(),
            plan: f.plan.clone(),
            admin_email: f.admin_email.clone(),
            admin_user: f.admin_user.clone(),
            error,
            success,
        }
        .into_response()
    ...
    if let Some(err) = validate_wizard(&f) {
        return Ok(render(Some(err), None, &f));
    }
    let pool = st
        .db
        .as_ref()
    ...
    match st.kc.create_user(&new_user).await {
        Ok(kc_id) => {
            ...
            Ok(render(
                None,
                Some(format!(
                    "tenant {} criado; KC user {} enviado CONFIGURE_TOTP",
                    tenant_id, kc_id
                )),
                &f,
            ))
        ...
        Err(e) => {
            ...
            let _ = sqlx::query("DELETE FROM tenants WHERE id = $1")
                ...
                .await;
            Ok(render(
                Some(format!("KC user create failed (tenant row reverted): {e}")),
                None,
                &f,
            ))
        }
...
mod tests {
    ...
    fn wizard_accepts_canonical_plans() {
        for p in PLANS {
            assert!(validate_wizard(&wizard("acme", p)).is_none(), "{p}");
        }
    }
