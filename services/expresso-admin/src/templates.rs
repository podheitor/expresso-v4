...
mod tests {
    ...
    #[test]
    fn kc_user_email_preserved() {
        let json = r#"{"id":"e1","email":"eva@corp.com"}"#;
        let u: KcUser = serde_json::from_str(json).unwrap();
        assert_eq!(u.email, "eva@corp.com");
    }

    #[test]
    fn kc_user_first_name_from_camel_case_key() {
        let json = r#"{"id":"f1","firstName":"Frank"}"#;
        let u: KcUser = serde_json::from_str(json).unwrap();
    ...
    #[test]
    fn kc_realm_registration_allowed_defaults_false() {
        let json = r#"{"realm":"r"}"#;
        let r: KcRealm = serde_json::from_str(json).unwrap();
        assert!(!r.registration_allowed);
    }
