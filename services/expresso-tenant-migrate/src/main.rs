...
fn roles_to_assign(source_roles: &[Value], dest_roles: &[Value]) -> Vec<Value> {
    let dest_by_name: std::collections::HashMap<&str, &Value> = dest_roles
        .iter()
...
async fn run(c: Client, cli: Cli, tok: String) -> Result<Summary> {
    ...
    for u in users {
        match extract_tenant_id(&u) {
            ...
            None => {
                sum.users_missing_tenant += 1;
                warn!(username = ?u["username"], "user missing tenant_id attribute");
            }
        }
    }
    ...
    for (tenant, tenant_users) in by_tenant.iter() {
        ...
        let dest_roles = list_realm_roles(&c, &tok, &cli.kc_url, tenant)
            .await
            .unwrap_or_default();
        ...
        for u in tenant_users {
            ...
            match user_exists(&c, &tok, &cli.kc_url, tenant, username).await {
                ...
                Ok(None) => {}
                Err(e) => {
                    warn!(%username, %tenant, error = %e, "user_exists failed");
                    ts.errors += 1;
                    continue;
                }
            }
            ...
            let new_id = match create_user(&c, &tok, &cli.kc_url, tenant, &payload).await {
                ...
                Err(e) => {
                    warn!(%username, %tenant, error = %e, "create_user failed");
                    ts.errors += 1;
                    continue;
                }
            };
            ...
            if let Some(source_id) = u["id"].as_str() {
                if let Ok(src_roles) =
                    user_realm_roles(&c, &tok, &cli.kc_url, &cli.source_realm, source_id).await
                {
                    let mapped = roles_to_assign(&src_roles, &dest_roles);
                    let n = mapped.len();
                    if let Err(e) =
                        assign_roles(&c, &tok, &cli.kc_url, tenant, &new_id, &mapped).await
                    {
...
mod tests {
    ...
    #[test]
    fn extract_tenant_id_absent_is_none() {
    ...
    #[test]
    fn extract_tenant_id_valid_value_returned() {
        let u = json!({"attributes": {"tenant_id": ["acme"]}});
        assert_eq!(extract_tenant_id(&u).as_deref(), Some("acme"));
    }
    ...
    #[test]
    fn roles_to_assign_empty_src_returns_empty() {
        let src: Vec<serde_json::Value> = vec![];
        let dst = vec![json!({"id": "1", "name": "admin"})];
        let result = roles_to_assign(&src, &dst);
