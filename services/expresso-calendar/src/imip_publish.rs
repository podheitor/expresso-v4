...
mod tests {
    ...
    #[test]
    fn envelope_method_field_matches_arg() {
        let ev = sample_event(ICAL_WITH_ATTENDEES);
        let bytes = build_envelope_bytes(&ev, "CANCEL").unwrap().unwrap();
        let v: serde_json::Value = serde_json::from_slice(&bytes).unwrap();
        assert_eq!(v["method"], "CANCEL");
    ...
    #[test]
    fn envelope_method_field_is_preserved() {
        let ev = sample_event(ICAL_WITH_ATTENDEES);
        let bytes = build_envelope_bytes(&ev, "CANCEL").unwrap().unwrap();
        let v: serde_json::Value = serde_json::from_slice(&bytes).unwrap();
    ...
    #[test]
    fn envelope_method_request_field_is_preserved() {
        let ev = sample_event(ICAL_WITH_ATTENDEES);
        let bytes = build_envelope_bytes(&ev, "REQUEST").unwrap().unwrap();
        let v: serde_json::Value = serde_json::from_slice(&bytes).unwrap();
    ...
    #[test]
    fn envelope_cancel_method_field_is_cancel() {
        let ev = sample_event(ICAL_WITH_ATTENDEES);
        let bytes = build_envelope_bytes(&ev, "CANCEL").unwrap().unwrap();
        let v: serde_json::Value = serde_json::from_slice(&bytes).unwrap();
        assert_eq!(v["method"].as_str(), Some("CANCEL"));
    ...
    #[test]
    fn envelope_invite_sequence_matches_event() {
        let ev = sample_event(ICAL_WITH_ATTENDEES);
        let bytes = build_envelope_bytes(&ev, "REQUEST").unwrap().unwrap();
        let v: serde_json::Value = serde_json::from_slice(&bytes).unwrap();
        assert_eq!(v["invite"]["sequence"], 2);
