...
mod tests {
    ...
    #[test]
    fn default_port_is_8100() {
        assert_eq!(DEFAULT_PORT, 8100);
    }

    #[test]
    fn service_name_is_expresso_auth() {
        assert_eq!(SERVICE, "expresso-auth");
    }
    ...
    fn resolve_addr_host_env_overrides_default() {
        ...
        std::env::remove_var("PORT");
        let addr = resolve_addr().unwrap();
        assert_eq!(addr.ip().to_string(), "10.0.0.100");
        std::env::remove_var("HOST");
    }
