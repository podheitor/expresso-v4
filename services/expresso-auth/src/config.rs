...
impl RpConfig {
    ...
    pub fn from_env() -> anyhow::Result<Self> {
        ...
        let issuer_template = std::env::var("AUTH_RP__ISSUER_TEMPLATE")
            .ok()
            .filter(|v| !v.is_empty());
        let redirect_uri_template = std::env::var("AUTH_RP__REDIRECT_URI_TEMPLATE")
            .ok()
            .filter(|v| !v.is_empty());
        let post_logout_template = std::env::var("AUTH_RP__POST_LOGOUT_TEMPLATE")
            .ok()
            .filter(|v| !v.is_empty());
        Ok(Self {
            issuer,
...
mod tests {
    ...
    fn redirect_uri_template_is_stored() {
        let c = cfg(
            ...
            "https://app/cb",
            None,
            Some("https://{host}/auth/callback"),
            None,
        );
        assert_eq!(
            c.redirect_uri_template.as_deref(),
            Some("https://{host}/auth/callback")
        );
    }
    ...
    fn issuer_template_is_stored() {
        let c = cfg(
            ...
            "cli",
            "https://app/cb",
            Some("https://kc/realms/{realm}"),
            None,
            None,
        );
        assert_eq!(
            c.issuer_template.as_deref(),
            Some("https://kc/realms/{realm}")
        );
    }
    ...
    fn post_logout_template_stored_when_provided() {
        ...
        assert_eq!(
            c.post_logout_template.as_deref(),
            Some("https://{host}/bye")
        );
    }
