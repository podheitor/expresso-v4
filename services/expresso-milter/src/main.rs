...
use expresso_mail_auth::{verify_inbound, DkimSignerState, MAIL_AUTH_ACTIONS_TOTAL};
...
async fn main() -> anyhow::Result<()> {
    ...
    let dkim_signer: Option<Arc<DkimSignerState>> = match (
        ...
        (Some(sel), Some(path)) => {
            match DkimSignerState::from_pem_file(&mail_domain, &sel, &path) {
                Ok(s) => Some(Arc::new(s)),
                Err(e) => {
            ...
            move |cx| {
                ...
                Box::pin(async move {
                    ...
                    if let Some(user) = auth_authen {
                            ...
                            match signer.sign(&raw) {
                                Ok(sig_header) => {
                                    ...
                                    if let Some((name, value)) = parse_header_line(&sig_header) {
                                        ...
                                        if let Err(e) =
                                            cx.actions.insert_header(0, name_c, value_c).await
                                        {
                        ...
                        let auth =
                            verify_inbound(ip, &session.helo, &session.mail_from, &domain, &raw)
                                .await;
    ...
    tokio::spawn(async move {
        ...
        let app: Router = Router::new()
            .route(
                "/health",
                get(|| async {
                    Json(serde_json::json!({"service":"expresso-milter","status":"ok"}))
                }),
            )
            .route(
                "/ready",
                get(|| async {
                    Json(serde_json::json!({"service":"expresso-milter","status":"ready"}))
                }),
            )
