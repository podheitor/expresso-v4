...
use crate::{
    templates::{
        AddressbookAdminEditTpl, AddressbooksAdminTpl, CalendarAdminEditTpl, CalendarsAdminTpl,
        DavRow,
    },
    AdminError, AppState,
};
...
async fn caller_tenant_scope(st: &AppState, headers: &axum::http::HeaderMap) -> Option<uuid::Uuid> {
    let p = crate::auth::principal_for(st, headers).await;
    if crate::auth::is_super_admin(&p.roles) {
        None
    } else {
        p.tenant_id
    }
...
pub async fn calendars_list(
    ...
    let rows = rows
        .into_iter()
        .map(|(id, tid, tname, oe, n, d, col, dflt, ct)| {
            to_dav_row(id, tid, tname, oe, n, d, col, dflt, ct)
        })
        .collect();
    Ok(CalendarsAdminTpl {
        current: "calendars",
        rows,
    })
}
...
pub async fn calendar_edit_action(
    ...
    let color = if f.color.trim().is_empty() {
        None
    } else {
        Some(f.color.trim().to_string())
    };
    sqlx::query(
        r#"UPDATE calendars
...
pub async fn calendar_delete_action(
    ...
    let actual: Option<(String,)> =
        sqlx::query_as("SELECT name FROM calendars WHERE tenant_id = $1 AND id = $2")
            .bind(tenant_id)
            .bind(id)
            .fetch_optional(pool)
            .await
    ...
    if f.confirm_name.trim() != name {
        crate::audit::record(
            ...
            "admin.calendar.delete.rejected",
            Some("calendar"),
            Some(id.to_string()),
            Some(400),
...
pub async fn addressbooks_list(
    ...
    let rows = rows
        .into_iter()
        .map(|(id, tid, tname, oe, n, d, dflt, ct)| {
            to_dav_row(id, tid, tname, oe, n, d, None, dflt, ct)
        })
        .collect();
    Ok(AddressbooksAdminTpl {
...
pub async fn addressbook_delete_action(
    ...
    let actual: Option<(String,)> =
        sqlx::query_as("SELECT name FROM addressbooks WHERE tenant_id = $1 AND id = $2")
            .bind(tenant_id)
            .bind(id)
            .fetch_optional(pool)
            .await
    ...
    if f.confirm_name.trim() != name {
        crate::audit::record(
            ...
            "admin.addressbook.delete.rejected",
            Some("addressbook"),
            Some(id.to_string()),
            Some(400),
            serde_json::json!({ "tenant_id": tenant_id, "reason": "confirm_name_mismatch" }),
