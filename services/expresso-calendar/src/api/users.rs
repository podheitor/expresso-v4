...
mod tests {
    ...
    #[test]
    fn user_query_email_is_none_on_absent_field() {
        let q: UserQuery = serde_json::from_str(r#"{}"#).unwrap();
        assert!(q.email.is_none());
    }

    #[test]
    fn user_query_email_nonempty_when_present() {
        let q: UserQuery = serde_json::from_str(r#"{"email":"someone@corp.io"}"#).unwrap();
        assert!(!q.email.as_deref().unwrap_or("").is_empty());
    }

    #[test]
    fn user_query_email_with_numbers_stored() {
    ...
    #[test]
    fn user_query_email_subdomain_stored() {
        let q: UserQuery = serde_json::from_str(r#"{"email":"dev@mail.sub.example.com"}"#).unwrap();
        assert_eq!(q.email.as_deref(), Some("dev@mail.sub.example.com"));
    }
}
