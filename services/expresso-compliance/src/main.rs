...
mod tests {
    ...
    fn resolve_addr_custom_port() {
        let _g = ENV_LOCK.lock().unwrap();
        std::env::set_var("PORT", "9800");
        std::env::remove_var("HOST");
        let addr = resolve_addr().unwrap();
        assert_eq!(addr.port(), 9800);
        std::env::remove_var("PORT");
    }
    ...
    #[test]
    fn default_port_is_8009() {
        assert_eq!(DEFAULT_PORT, 8009);
    }

    #[test]
    fn service_name_is_expresso_compliance() {
        assert_eq!(SERVICE, "expresso-compliance");
    }
    ...
    #[test]
    fn default_retention_interval_is_3600() {
        assert_eq!(DEFAULT_RETENTION_INTERVAL, 3600);
    }

    #[test]
    fn resolve_addr_custom_port_8009() {
        let _g = ENV_LOCK.lock().unwrap();
        std::env::set_var("PORT", "8009");
        std::env::remove_var("HOST");
        let addr = resolve_addr().unwrap();
