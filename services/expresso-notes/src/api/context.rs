...
//!
//! Mode 1 (strict, production): an `Arc<OidcValidator>` (or multi-realm
//! validator + tenant resolver) in request extensions → require a Bearer token,
//! validate it, derive tenant/user from claims.
//! Mode 2 (dev fallback): read `X-Tenant-Id` / `X-User-Id` headers when no
//! validator is wired.
...
impl<S: Send + Sync> FromRequestParts<S> for RequestCtx {
    ...
    async fn from_request_parts(parts: &mut Parts, _: &S) -> Result<Self, Self::Rejection> {
        // Multi-realm precedence: MultiRealmValidator + TenantResolver present →
        // resolve realm via Host header and validate.
