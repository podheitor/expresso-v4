...
pub fn routes() -> Router<AppState> {
    Router::new()
        .route(
            "/api/v1/addressbooks/:book_id/acl",
            get(list_acl).post(share),
        )
        .route(
            "/api/v1/addressbooks/:book_id/acl/:grantee_id",
            delete(revoke),
        )
}
...
async fn assert_owner(
    ...
    let row: Option<(Uuid,)> =
        sqlx::query_as("SELECT owner_user_id FROM addressbooks WHERE id = $1 AND tenant_id = $2")
            .bind(book_id)
            .bind(tenant_id)
            .fetch_optional(pool)
            .await?;
